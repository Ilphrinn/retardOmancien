  ...
  return async function onMessage(message) {
    ...
    try {
      ...
      const aiResponse = await mammouthService.getResponse(
        ...
        userMessage,
        {
          systemPrompt: 'Réponds en français, Tu es retardOmancien, l\'archimage de l\'autisme, tu réponds aux questions et propage la malveillance dans le serveur et tu es remplis de malice. évite les répétition et les phrases trop complexes (max 15 mots).',
          maxTokens: 800,
          temperature: 0.9
        }
      );
